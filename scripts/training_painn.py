...
def train_painn(
    *,
    ...
    modelparams_override: Optional[dict] = None,
):
    ...
    if modelparams_override:
        modelparams.update(modelparams_override)

    os.makedirs(outdir, exist_ok=True)
    best_model_path = os.path.join(outdir, "best_model")
    ...
    train_loader = DataLoader(
        train, batch_size=batch_size, collate_fn=collate_dicts, sampler=RandomSampler(train)
    )
    ...
    optimizer = Adam(trainable_params, lr=1.5e-4)
    ...
    results, targets, val_loss = evaluate(trainer.get_best_model(), test_loader, loss_fn, device=device)
    ...
    if plot_results:
        ...
        for ax, key in zip(ax_fig, units.keys()):
            ...
            ax.set_xlabel(f"Predicted {key} ({units[key]})")
            ax.set_ylabel(f"Target {key} ({units[key]})")

        fig.tight_layout()

    return {
        "outdir": outdir,
        "best_model": best_model_path if os.path.isfile(best_model_path) else None,
        "val_loss": val_loss,
    }
...
if __name__ == "__main__":
    train_painn(
        train_path="/path/to/train_dataset.pth.tar",
        val_path="/path/to/val_dataset.pth.tar",
        test_path="/path/to/test_dataset.pth.tar",
    )
